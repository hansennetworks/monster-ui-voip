define(function (require) {
    ...
    var app = {
        ...
        subscribe: {
            'voip.licenses.render': 'licensesRender'
        },
        ...
        licensesRender: function (args) {
            ...
            self.usersGetData(function (data) {
                var dataTemplate = self.usersFormatListData(data),
					template = $(monster.template(self, 'licenses-layout', dataTemplate)),
					templateUser;

                _.each(dataTemplate.users, function (user) {
                    templateUser = monster.template(self, 'licenses-row', user);

                    template.find('.licenses-rows').append(templateUser);
                });
                ...
                if (dataTemplate.users.length == 0) {
                    ...
                    parent.find('.no-licenses-row').css('display', 'none');
                }
